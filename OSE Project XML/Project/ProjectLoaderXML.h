...
namespace ose::project
{
	...
	public:
		...
		uptr<Project> LoadProject(const std::string & project_name);
		uptr<ProjectInfo> LoadProjectManifest(const std::string & project_path);

		uptr<std::map<std::string, std::string>> LoadSceneDeclerations(const std::string & project_path);
		uptr<Tag> LoadTagDefinitions(const std::string & project_path);
		...
		uptr<Scene> LoadScene(const Project & project, const std::string & scene_name);
		void LoadChunk(Chunk & chunk, Project const & project);
		uptr<Entity> LoadEntityPrefab(const std::string & prefab_path, const Project & project);

		uptr<CustomObject> LoadCustomDataFile(const std::string & path);
	...
	private:
		...
		uptr<rapidxml::xml_document<>> LoadXmlFile(const std::string & path, std::string & contents);
