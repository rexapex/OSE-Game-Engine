...
namespace ose::project
{
	...
	public:
		...
		uptr<std::map<std::string, std::string>> LoadSceneDeclerations(std::string const & project_path);
		uptr<Tag> LoadTagDefinitions(std::string const & project_path);
	...
	private:
		...
		void ParseResources(rapidxml::xml_node<> * resources_node, std::unordered_map<std::string, std::string> & aliases, Project const & project);
