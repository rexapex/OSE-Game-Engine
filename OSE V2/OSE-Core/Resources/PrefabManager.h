...
namespace ose::resources
{
	...
	public:
		...
		entity::Entity & AddTempPrefab(Args &&... params, std::string const & path);

		// add a temporary prefab entity to the prefab manager
		// method moves the object passed
		void AddTempPrefab(std::unique_ptr<entity::Entity> e, std::string const & path);

		// add a persistent (cached) prefab entity to the prefab manager
		// method constructs a new object
		// method takes an array of constructor arguments
		template<typename... Args>
		entity::Entity & AddCachedPrefab(Args &&... params, std::string const & path);

		// add a persistent (cached) prefab entity to the prefab manager
		// method moves the object passed
		void AddCachedPrefab(std::unique_ptr<entity::Entity> e, std::string const & path);

		// get a reference to the entity prefab with path given
		// checks both temporary and cached entities
		entity::Entity & GetPrefab(std::string const & path);
	...
	private:
		...
		std::unordered_map<std::string, std::unique_ptr<entity::Entity>> temp_prefabs_;
		...
		std::unordered_map<std::string, std::unique_ptr<entity::Entity>> cached_prefabs_;
	};
}
