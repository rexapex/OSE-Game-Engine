...
namespace ose::shader
{
	...
	public:
		...
		ose::unowned_ptr<ShaderNode> AddNode(Args &&... params)
		{
		...
		bool RemoveNode(ose::unowned_ptr<ShaderNode> node);

		// Connect the 2 nodes passed together
		// Returns true iff the nodes were connected
		bool Connect(ose::unowned_ptr<ShaderNode> out_node, std::string const & out_node_output, ose::unowned_ptr<ShaderNode> in_node, std::string const & in_node_input);
		...
		ose::unowned_ptr<ShaderNode> const GetOutputColorNode() const { return nodes_[0].get(); }

		// Get a list of all nodes which connect to into node n
		std::vector<ose::unowned_ptr<NodeConnector>> const GetIncomingConnectors(ose::unowned_ptr<ShaderNode> n);
