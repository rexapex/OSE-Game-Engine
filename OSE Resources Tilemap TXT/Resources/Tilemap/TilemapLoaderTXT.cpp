...
namespace ose::resources
{
	...
	void TilemapLoaderTXT::LoadTilemap(const std::string & path, Tilemap & tilemap)
	{
		if(FileHandlingUtil::DoesFileExist(path))
		{
			...
			{
				LOG_ERROR("Error: Failed to load tilemap", path);
				return;
			}
			...
			for(size_t y = 0; y < lines.size(); y++)
			{
				...
				while(std::getline(line_ss, cell, ' '))
				{
					if(cell.size() > 0)
					{
						...
						if(x >= width)
						{
							LOG_ERROR("Error: Invalid tilemap file, varying row length");
						...
						{
							LOG_ERROR("Error: Invalid tilemap file, cell is not a number");
							tilemap.DeleteTileGrid();
							return;
						}
		...
		{
			LOG_ERROR("Error: Tilemap", path, "does not exist");
		}
	}
}
